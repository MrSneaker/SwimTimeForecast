...
def test(save_figures=True, show_figures=True, model_version="v4", use_optimized=False):
    ...
    if model_version.lower() == "v4":
        ...
        model_path = "../models/swim_tft_v4.pt"
        params = {
    ...
    with torch.no_grad():
        for batch_data, y_batch in tqdm(test_loader, desc=f"Testing {model_version}"):
            ...
            if model_version.lower() == "v4":
                # batch_data est un dictionnaire {'past': {...}, 'future': {...}, 'static': {...}}
                ...
                # Feature Importance (si implémenté dans ton VSN)
                # Note: Le format exact dépend de ton implémentation de 'feat_weights'
                if isinstance(feat_weights, dict): # Si retour par type
